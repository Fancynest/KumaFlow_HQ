...
object AppStr {
    ...
    val recordExpenseBtn get() = if(isId) "Catat ke Pengeluaran" else "Record to Expenses"
    ...
    val selectCategory get() = if(isId) "Pilih Kategori" else "Select Category"
    val txNameLabel get() = if(isId) "Nama Transaksi" else "Transaction Name"
    val youUserBadge get() = if(isId) "Anda (Pengguna)" else "You (User)"
    val recordedToExpensesHint get() = if(isId) "• Dicatat ke Expenses" else "• Recorded to Expenses"
    val nameYouLabel get() = if(isId) "Nama (Anda)" else "Name (You)"
    val nameLabel get() = if(isId) "Nama" else "Name"
    val nominalLabel get() = if(isId) "Nominal" else "Amount"
    val selectAmountToRecord get() = if(isId) "Pilih nominal yang ingin dicatat:" else "Select amount to record:"
    val grandTotal get() = if(isId) "Total Keseluruhan" else "Grand Total"
    val plusTax get() = if(isId) "+ Pajak" else "+ Tax"
    val defaultMe get() = if(isId) "Anda" else "You"
    val defaultFriend get() = "Person"
    val totalBillExample get() = if(isId) "Contoh: 150000" else "e.g. 150000"
    val deletePersonContentDesc get() = if(isId) "Hapus Orang" else "Delete Person"
    val deleteNominalContentDesc get() = if(isId) "Hapus Nominal" else "Delete Amount"
    ...
    val scanReceiptFailed get() = if(isId) "Gagal membaca struk, silakan isi manual" else "Failed to parse receipt, please fill manually"
    ...
    val ocrApiKeyRequiredDesc get() = if(isId) "Fitur Scan Struk AI memerlukan API Key (Google Gemini atau Anthropic Claude). Masukkan API Key Anda di bawah." else "AI Receipt Scan requires an API Key (Google Gemini or Anthropic Claude). Please enter your API Key below."
    val ocrSettingsTitle get() = if(isId) "AI & OCR Struk Belanja" else "AI & Receipt OCR"
    val ocrProviderLabel get() = if(isId) "Penyedia Model AI" else "AI Model Provider"
    val ocrProviderAnthropic get() = "Anthropic (Claude)"
    val ocrProviderGemini get() = "Google (Gemini)"
    val ocrApiKeyLabel get() = "Anthropic API Key (sk-ant-...)"
    val ocrApiKeyHint get() = if(isId) "Model: claude-haiku-4-5. Kunci Anda disimpan aman & terenkripsi lokal di Android Keystore.\n\nStatus: Berbayar (Memerlukan saldo kredit di Anthropic)." else "Model: claude-haiku-4-5. Your key is stored securely & encrypted locally in Android Keystore.\n\nStatus: Paid (Requires credit balance on Anthropic)."
    val ocrGeminiKeyLabel get() = "Gemini API Key (AIzaSy...)"
    val ocrGeminiKeyHint get() = if(isId) "Model: gemini-3.1-flash-lite. Kunci disimpan aman & terenkripsi lokal di Keystore.\n\nStatus: 100% Gratis (Tersedia Free Tier tanpa kartu kredit)." else "Model: gemini-3.1-flash-lite. Key is securely stored & encrypted locally in Keystore.\n\nStatus: 100% Free (Free Tier available, no credit card required)."
    val ocrOpenAiStudio get() = if(isId) "Buka aistudio.google.com" else "Open aistudio.google.com"
    val ocrOpenAnthropicConsole get() = if(isId) "Buka console.anthropic.com" else "Open console.anthropic.com"
    val ocrApiKeySaved get() = if(isId) "API Key berhasil disimpan" else "API Key saved successfully"
    val ocrApiKeyDeleted get() = if(isId) "API Key berhasil dihapus" else "API Key deleted"
    val ocrApiKeyNotConfigured get() = if(isId) "Belum dikonfigurasi" else "Not configured"
    ...
    val resetBal get() = if(isId) "Reset Total Saldo" else "Reset Total Balance"
    val resetBalConfTitle get() = if(isId) "Reset Saldo?" else "Reset Balance?"
    val resetBalConfDesc get() = if(isId) "Ini akan membuat transaksi penyeimbang baru agar saldo menjadi 0 tanpa menghapus riwayat transaksi lama. Anda yakin?" else "This will create a balancing transaction to reset your balance to 0 without deleting your transaction history. Are you sure?"
    val resetOk get() = if(isId) "Saldo berhasil direset" else "Balance reset successfully"
    ...
    val dobLbl get() = if(isId) "Tanggal Lahir (DD-MM-YYYY)" else "Date of Birth (DD-MM-YYYY)"
    val easterEggLbl get() = if(isId) "Kode Easter Egg" else "Easter Egg Code"
    ...
    val newWallet get() = if(isId) "Kartu Baru" else "New Card"
    val walletName get() = if(isId) "Nama Kartu" else "Card Name"

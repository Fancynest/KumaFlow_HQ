...
object AppStr {
    ...
    val ocrProviderGemini get() = if(isId) "Google (Gemini) • Gratis" else "Google (Gemini) • Free"
    val ocrApiKeyLabel get() = "Anthropic API Key (sk-ant-...)"
    val ocrApiKeyHint get() = if(isId) "Dapatkan API Key di console.anthropic.com. Kunci Anda disimpan aman dan terenkripsi secara lokal di Android Keystore." else "Get your API Key from console.anthropic.com. Your key is stored securely & encrypted locally in Android Keystore."
    val ocrGeminiKeyLabel get() = "Gemini API Key (AIzaSy...)"
    val ocrGeminiKeyHint get() = if(isId) "Dapatkan API Key gratis di aistudio.google.com. Menggunakan model gemini-3.1-flash-lite. Kunci disimpan aman & terenkripsi lokal di Keystore." else "Get your free API Key from aistudio.google.com. Powered by gemini-3.1-flash-lite. Key is securely encrypted in Android Keystore."

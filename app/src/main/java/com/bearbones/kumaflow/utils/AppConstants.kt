...
object AppStr {
    ...
    val ocrApiKeyRequiredTitle get() = if(isId) "API Key Anthropic Diperlukan" else "Anthropic API Key Required"
    val ocrApiKeyRequiredDesc get() = if(isId) "Fitur Scan Struk AI memerlukan API Key Anthropic (Claude). Silakan masukkan API Key Anda di Pengaturan." else "AI Receipt Scan requires an Anthropic (Claude) API Key. Please enter your API Key in Settings."
    val ocrSettingsTitle get() = if(isId) "AI & OCR Struk (Anthropic)" else "AI & Receipt OCR (Anthropic)"
    val ocrApiKeyLabel get() = if(isId) "Anthropic API Key (sk-ant-...)" else "Anthropic API Key (sk-ant-...)"
    ...
    val ocrApiKeySaved get() = if(isId) "API Key Anthropic berhasil disimpan" else "Anthropic API Key saved successfully"
    val ocrApiKeyDeleted get() = if(isId) "API Key Anthropic berhasil dihapus" else "Anthropic API Key deleted"
